/**
 * Cellar's half of `public.user_settings`.
 *
 * The table is Radar's (docs/shared-database.md). Cellar reads the row and
 ...
 * shelf and the cellar together.
 *
 * `currentStreak` / `streakUpdatedAt` are read-only here. They are Radar's film
 * streak, published for its own notification generator, and Cellar borrows them
 * for the cross-app strip on Today (lib/siblingStreaks). Writing them would
 * overwrite Radar's figure with an entry count and silently break its 20:00
...
/** The only columns Cellar is allowed to write. Enforced, not documented. */
